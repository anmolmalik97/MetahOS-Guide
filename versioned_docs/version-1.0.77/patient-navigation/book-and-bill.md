...
## Booking an appointment from Patient Navigation

The **Book Appointment** action on the navigation screen is a dropdown, so the right kind of appointment is chosen up front rather than through separate buttons:

| Option | Books |
| --- | --- |
| **Book Appointment** | A standard OPD consultation appointment. |
| **Book Surgery Appointment** | A surgery slot. |
| **Book Rad Appointment** | A radiology appointment. Shown only where radiology is available. |

:::note
**Book Surgery Appointment** is disabled until an **assigning authority** (site) is selected — a surgery slot has to belong to a specific facility. The dropdown says so on the option itself, so it is clear why it cannot be picked.
:::
