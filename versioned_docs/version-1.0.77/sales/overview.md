...
- **Proforma Invoices** — formal priced documents the customer can raise a purchase order or an advance against, before any tax invoice exists.
- **Orders** — confirmed sales orders that reserve stock and move through a receive / confirm / approve lifecycle.
- **Delivery** — packing, dispatch, delivery challans and delivery notes.
...
- **Returns** — goods sent back, and the credit notes that settle them.
- **Price Lists** — the agreed rates that apply to a customer.
- **Reports** — a summary of the pipeline across every document type.
...
## Phase 3 — Fulfilment, pricing and control

Phase 3 extended the module from order-to-cash into full fulfilment, and added the commercial controls around it:

- **Proforma invoices** with their own approval steps, validity period and advance-payment tracking. See [Proforma Invoices](./proforma-invoices.md).
- **Delivery** — packing, dispatch, delivery challans and delivery notes, with partial dispatch. See [Delivery & Returns](./delivery-and-returns.md).
- **Returns and credit notes** for goods that come back.
- **Price lists** per customer, **discount approval** and **credit limits** with a permissioned override. See [Price Lists, Discounts & Credit Control](./pricing-and-credit-control.md).
- **Audit history** on every document, and a **Reports** tab over the pipeline. See [Sales Reports & Audit History](./sales-reports-and-audit.md).
- **Customer sales history** — every document and transaction for a customer in one view.
